...
export default async function ReimbursementsPage() {
  ...
  async function fetchTeamRequests(): Promise<(Reimbursement & { employee: { full_name: string; email: string } | null })[]> {
    if (!canReview) return []
  ...
  const [{ data: myRequests }, teamRequests] = await Promise.all([
    supabase.from("reimbursements").select("*").eq("employee_id", employee.id).order("created_at", { ascending: false }),
    fetchTeamRequests(),
  ])
